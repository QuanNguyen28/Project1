...
def _schema() -> CollectionSchema:
    ...
    fields = [
        FieldSchema(
            name=FIELD_CHUNK_ID, dtype=DataType.VARCHAR, max_length=64, is_primary=True
        ),
    ...
    return CollectionSchema(
        fields, description="JD chunks for semantic search (COSINE)"
    )


def ensure_collection(
    index_params: Optional[dict] = None, *, load: bool = True
) -> Collection:
