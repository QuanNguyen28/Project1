...
def upsert_jd_from_markdown(
    ...
    *,
    ...
    job_code = slugify(
        meta.get("job_code") or meta.get("title") or os.path.basename(path), max_len=50
    )
    ...
    if row is None:
        ...
        row = (
            db.query(JobDescription).filter(JobDescription.job_code == job_code).first()
        )
...
if __name__ == "__main__":
    ...
    ap = argparse.ArgumentParser(
        description="ETL JD markdown -> Postgres (job_descriptions + jd_versions + jd_tag_map)"
    )
    ...
    ap.add_argument(
        ...
        type=str,
        default=JD_DIR,
        help="Thư mục JD .md (mặc định: etl/jd_markdown)",
    )

    ap.add_argument("--author", type=str, default="etl")

    ap.add_argument(
        "--no-update", action="store_true", help="Không cập nhật JD đã tồn tại (skip)"
    )

    ap.add_argument("--no-tags", action="store_true", help="Không ghi JD tag map")

    ap.add_argument(
        "--summary", type=str, default=None, help="change_summary cho jd_versions"
    )
    ...
    ap.add_argument(
        "--recursive",
        action="store_true",
        help="Quét đệ quy **/*.md (mặc định: chỉ *.md ở gốc)",
    )
