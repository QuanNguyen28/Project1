...
def _format_chunks_text(chunks, max_chars: int = 3500) -> str:
    ...
    for i, c in enumerate(chunks, 1):
        ...
        if not body:
            ...
            body = (
                f"(No snippet; source_path={getattr(c, 'object_path', None) or 'N/A'})"
            )
...
def create_jd_endpoint(
    ...
    snippet_lines: int = Query(
        12, ge=1, le=100, description="Lines to preview from each chunk file"
    ),
):
    """
    ...
    try:
        ...
        family_id = (
            get_or_create_family(db, req.job_family)
            if getattr(req, "job_family", None)
            else None
        )

        jd_id = create_jd(
            db, req=req, created_by=current_user.username, family_id=family_id
        )

        user_chunks_list = (req.chunks or []) if hasattr(req, "chunks") else []

        user_chunks_text = "\n\n---\n".join(
            [str(c).strip() for c in user_chunks_list if c and str(c).strip()]
        )
        ...
        return JDGenerateResponse(
            jd_id=jd_id, content_md=content_md, version=version_number
        )
...
def suggest_jd_endpoint(
    ...
    try:
        ...
        return JDSuggestResponse(
            suggestions=out["bullets"], rationale=out.get("rationale")
        )
...
def improve_stream(
    ...
    try:
        ...
        return StreamingResponse(
            stream_text_lines(improved), media_type="text/event-stream"
        )
