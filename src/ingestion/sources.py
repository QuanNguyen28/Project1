...
USER_AGENT = "SmartHireResearchBot/1.0 (public job ingestion; contact your-org@example.com)"
...
class JobPosting:
    ...
    def content_hash(self) -> str:
        payload = "\n".join((self.title, self.company, self.description, self.source_url))
    ...
    def fetch(self, limit: Optional[int] = None) -> list[JobPosting]:
        ...
        for job in jobs:
            ...
            output.append(JobPosting(
                ...
                department=", ".join(str(item.get("name")) for item in departments if item.get("name")),
    ...
    def fetch(self, limit: Optional[int] = None) -> list[JobPosting]:
        ...
        for job in jobs:
            ...
            output.append(JobPosting(
                ...
                description=str(job.get("descriptionPlain") or _plain_text(job.get("description"))),
...
class JsonLdSource(BaseSource):
    ...
    def fetch_urls(self, urls: Iterable[str], limit: Optional[int] = None) -> list[JobPosting]:
        output: list[JobPosting] = []
        for page_url in urls:
            response = self.session.get(page_url, timeout=30, headers={"Accept": "text/html"})
            response.raise_for_status()
            ...
            for node in soup.select('script[type="application/ld+json"]'):
                ...
                for item in candidates:
                    ...
                    external_id = str(item.get("identifier", {}).get("value") or item.get("url") or page_url)
                    output.append(JobPosting(
                        source_name="jsonld",
                        ...
                        source_url=urljoin(page_url, str(item.get("url") or page_url)),
