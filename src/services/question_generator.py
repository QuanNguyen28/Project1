...
def _render_prompt(ctx: GenerationContext) -> str:
    ...
    try:
        ...
        _logger.warning(
            "Template 'interview_questions.j2' not found. Using fallback prompt."
        )
...
def _mmr_select(
    candidates: List[Question], k: int, lambda_div: float = 0.75
) -> List[Question]:
...
def _offline_generate(ctx: GenerationContext, seed: int) -> GenResult:
    ...
    for i in range(max(1, ctx.count * 3)):
        ...
        q = tpl.format(
            title=ctx.title, level=ctx.level, department=ctx.department, focus=f
        )
        ...
        cands.append(
            Question(
                ...
                competency=(
                    f
                    if t == "technical"
                    else ("communication" if t == "behavioral" else "problem solving")
                ),
...
def generate_questions(
    ...
    provider_has_key = (LLM_PROVIDER == "gemini" and bool(GEMINI_API_KEY)) or (
    ...
    if LLM_BYPASS or not provider_has_key:
        ...
        result = GenResult(
            ...
            meta=GenMeta(
                ...
                seed=seed,
                elapsed_ms=0,
                dedup_ratio=1.0 - len(uniq) / max(1, len(qs)),
                coverage=cov,
            ),
        )
