...
def _validate_manifest() -> None:
    ...
    try:
        ...
        if manifest is None:
            raise RuntimeError("RAG index manifest is missing; run the indexing command")
...
def _eligible_jd_ids(company: Optional[str], department: Optional[str]) -> Optional[List[int]]:
    ...
    try:
        ...
        if company:
            query = query.filter(func.lower(JobDescription.source_company) == company.strip().lower())
        if department:
            query = query.filter(JobDescription.department.ilike(f"%{department.strip()}%"))
...
def _lexical_candidates(
    ...
    # websearch_to_tsquery treats a leading hyphen as NOT. Job titles commonly
    # use dashes as separators, so normalize them before parsing the query.
...
def _select_diverse(
    ...
    while remaining and len(selected) < top_k:
        allowed = [
            ...
            if counts.get(item.jd_id, 0) < max_chunks_per_jd
        ]
        if not allowed:
            break
        best = max(
            ...
            - 0.08 * max((_jaccard(item.content, prev.content) for prev in selected), default=0.0),
        )
...
def retrieve_dense(
    ...
    *,
    ...
    candidates = _hydrate(_dense_candidates(
        query_vec,
...
def retrieve_hybrid(
    ...
    *,
    ...
    dense = [] if mode == "lexical" else _dense_candidates(
        query_vec or [], max(top_k * 4, RAG_DENSE_CANDIDATES), company, department
    )
    lexical = [] if mode == "dense" else _lexical_candidates(
    ...
    if mode == "dense":
        ...
        maximum = max((item.lexical_score or 0.0 for item in candidates), default=1.0) or 1.0
        for item in candidates:
