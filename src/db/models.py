...
class JDTag(Base):
    ...
    parent_tag_id = Column(
        Integer, ForeignKey("jd_taxonomy_tags.tag_id"), nullable=True
    )

    children = relationship("JDTag", backref="parent", remote_side=[tag_id])

    job_descriptions = relationship(
        "JobDescription", secondary=jd_tag_map, back_populates="tags"
    )
...
class JobDescription(Base):
    ...
    __mapper_args__ = {"primary_key": (jd_id,)}
    ...
    tags = relationship(
        "JDTag", secondary=jd_tag_map, back_populates="job_descriptions"
    )

    chunks = relationship(
        "JDChunk", back_populates="job_description", cascade="all, delete-orphan"
    )
...
class JDChunk(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "jd_id", "chunk_index", name="jd_chunks_jd_id_chunk_index_key"
        ),
...
class JDVersion(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "jd_id", "version_number", name="jd_versions_jd_id_version_number_key"
        ),
        {"extend_existing": True},
    )
    ...
    jd_id = Column(
        Integer,
        ForeignKey("job_descriptions.jd_id", onupdate="CASCADE", ondelete="CASCADE"),
        nullable=False,
    )
...
class CandidateSkill(Base):
    ...
    candidate_id = Column(
        Integer, ForeignKey("candidate_profiles.candidate_id"), primary_key=True
    )
...
class Skill(Base):
    ...
    candidates = relationship(
        "CandidateProfile", secondary=candidate_skills, back_populates="skills"
    )
